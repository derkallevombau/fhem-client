...
class FhemClient {
    constructor(options, logger) {
        this.getOptions = { rejectUnauthorized: false };
    ...
    obtainCsrfToken() {
        ...
        return this.getWithPromise((res, resolve, reject) => {
            ...
            switch (res.statusCode) {
                ...
                case 200:
                    if (token)
                        logger.debug('obtainCsrfToken: Succeeded.');
                    else
                        logger.warn("obtainCsrfToken: No CSRF token received. Either this FHEMWEB doesn't use it, or it doesn't send it. We will see...");
                    resolve(token);
                    break;
                default:
    ...
    getWithPromise(processResponse) {
        ...
        return new Promise((resolve, reject) => {
            const req = this.client.get(this.url, this.getOptions, res => {
                ...
                res.on('error', e => error(`execCmd: Response error: Code: ${getErrorCode(e)}, message: ${e.message}`, 'RES', reject)).on('aborted', () => error('execCmd: Response closed prematurely.', 'ABRT', reject)).on('close', () => logger.info('execCmd: Connection closed.'));
